		...
		*/
		...
		function draw() {
			...
			for (var i = 0; i < nDots; i++) {
				...
				ctx.fill();
				//console.log(apertureCenterX/2)
				//console.log(apertureCenterY/2)
				if ( (dot.x <= 2*apertureCenterX/3) & (apertureCenterX/3 <= dot.x) & (found ==0) & (dot.y <= 2*apertureCenterY/3) & (apertureCenterY/3 <= dot.y) ){ //(dot.x <= 2*apertureCenterX/3) & (apertureCenterX/3 <= dot.x) & 
					found = 1
					ctx.fillStyle = "red";
					ctx.fill();
					console.log("x:", dot.x, "y:", dot.y)
					console.log(apertureCenterX/2 <= dot.x)
					console.log(2*apertureCenterY/3, apertureCenterY/3)
				}
			}
